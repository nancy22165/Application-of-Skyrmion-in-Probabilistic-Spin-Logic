...
for i=1:10000
    I2(i) = I0*(-m1(i));
    m2(i) = mi(lookup(d,I2(i)));
    I1(i+1) = I0*(- m2(i));
    m1(i+1) = mi(lookup(d,I1(i+1)));
end
I2(10001) = I0*(-m1(10001));
m2(10001) = mi(lookup(d,I2(10001)));
...
combined_bits =  m1_out * 2 + m2_out; % Assuming m1, m2, m3 are binary values

% Count occurrences of each combination
counts = histcounts(combined_bits, -0.5:3.5);

% Calculate probabilities
probabilities = counts / sum(counts) * 100;

% Create a histogram for the probabilities
bar(0:3, probabilities);
xlabel('p-bit code[A Y]');
